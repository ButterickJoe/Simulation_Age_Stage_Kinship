...
foreach(foc_IC = 3 )%do%{
  ...
  foreach(kin = kin_description)%do%{
    ...
    foreach(age_focal = 0 : (no_age-1))%do%{
      x_inx <- age_focal + 1
      X[, (x_inx)] <- 
        project_backwards_1_forwards_1_gen_over_steps_O(no_age, no_stage, 22, foc_IC, age_focal, P_FF, P_UU, lit_FF, lit_UU)
    }
...
full_older_sis <- do.call("rbind" , init_cond_kin1)
full_older_sis%>%head()
full_older_sis$ic_state_focal
...
df_my_variant <- full_older_sis%>%dplyr::select(full_dist,age_foc,kin_age,kin_stage)%>%
...
df_my_variant <- df_my_variant%>%
  transmute(Age_foc = age_foc, cum_kin = full_dist, stage = kin_stage,
            kin = "older sisters" ,method = "matrix model")%>%
...
saveRDS(df_my_variant, file = paste0(df_out , "/" , "OS_MatrixResult_IC3_stablePopStruct.Rds" ))
