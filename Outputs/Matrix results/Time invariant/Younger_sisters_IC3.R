...
####################### Genealogical matrix model ###
################ Time invariance means we simply fill lists with the same projection and genealogical matrices
lit_UU <- list()
lit_FF <- list()
for(i in 1:50){
  lit_FF[[i]] <- F_proj
...
############ Apply the model for each age of Focal ############################
init_cond_kin1 <- list() ## conditioning of Focal's initial stage
foreach(foc_IC = 3 )%do%{ ## here we can loop foc_IC = c(1,2,3,... ) if we want to compare all initial conditions of Focal
  ...
  init_cond_kin1[[(foc_IC)]] <- list_of_younger_sisters_new } ## end of IC loop 
full_younger_sis <- do.call("rbind" , init_cond_kin1)

## Put the results into a nice data frame for plotting #############
...
######### Save for comparison to simulation
saveRDS(df_my_variant, file = paste0(df_out , "/" , "YS_matrix_IC3.Rds" ))
